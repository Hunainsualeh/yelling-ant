        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {currentQuestion.options?.map((opt: QuizAnswer) => (
            ...
            >
              ...
              >
                {opt.label && (
                  <div className="whitespace-nowrap w-full max-w-[121px] text-[14px] sm:text-[16px] leading-[24px] text-[#1E2939] text-center overflow-visible">
                    {opt.label}
                  </div>
                )}
                <div className="whitespace-nowrap w-full max-w-[246px] text-[14px] sm:text-[16px] leading-[24px] text-[#696F79] text-center overflow-visible">
                  {opt.text}
                </div>
              </div>
            </button>
        ...
        <div className="flex flex-col gap-3">
          {currentQuestion.options?.map((opt: QuizAnswer) => (
            <button
              key={opt.id}
              onClick={() => handleClick(opt.id)}
              className={`w-full text-left p-5 md:p-6 rounded-xl border-2 transition-all duration-200 flex items-start gap-4 ${
